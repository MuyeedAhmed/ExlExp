...
def input_fn(request_body: Any, content_type: str = "application/json") -> Any:
    if content_type == "application/json":
        ...
        text_data = data.get("text") or data.get("ocr_text")
        ...
        return {"image": pil_img, "text": text_data}

    elif content_type in ["image/jpeg", "image/png", "image/webp", "application/octet-stream"]:
        if isinstance(request_body, str):
            request_body = request_body.encode("latin1")
        return {"image": Image.open(io.BytesIO(request_body)).convert("RGB"), "text": None}
...
def predict_fn(input_data: Any, model: Any) -> Dict[str, Any]:
    ocr_lines = []
    image = None
    ...
    if not ocr_lines and image is not None:
        try:
            import pytesseract
    ...
    full_ocr_text = "\n".join(ocr_lines)
    card_usage = extract_card_usage(full_ocr_text)
